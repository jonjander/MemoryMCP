...
public class MemoryTools(MemoryStoreService memoryStore, RefIdResolver refResolver)
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var result = await memoryStore.CreateMemoryAsync(resolvedRaw, memoryFrom, cancellationToken);
    ...
    [McpServerTool, Description(
        "Get a memory with linked entities, tokens, relationships, and revision history. " +
        "Large raw text is returned in slices (default max 32000 chars). Use rawOffset/rawMaxChars for pagination.")]
    public async Task<string> GetMemory(
        [Description(RefIdResolver.IdOrRefDescription)] string id,
        [Description("Character offset into raw text for paged reads of large memories.")] int rawOffset = 0,
        [Description("Maximum characters of raw text to return (default 32000).")] int? rawMaxChars = null,
        CancellationToken cancellationToken = default)
    {
        var memoryId = await refResolver.ResolveMemoryIdAsync(id, cancellationToken);
        var result = await memoryStore.GetMemoryAsync(memoryId, rawOffset, rawMaxChars, cancellationToken);
        if (result is null)
            return JsonResult.Ok(new { error = "Memory not found." });

        var steps = AgentGuidance.StepsForTruncatedRaw(result);
        return steps.Count == 0 ? JsonResult.Ok(result) : JsonResult.OkWithNextSteps(result, steps);
    }
    ...
    public async Task<string> ReviseMemory(
        ...
        [Description("Corrected observation text stored as a new memory. For large text, write a file and pass correctedRawPath instead.")] string? correctedRaw = null,
        ...
        [Description("Absolute or relative path to a UTF-8 text file with corrected raw text. Overrides correctedRaw when set. Max 2 MB.")] string? correctedRawPath = null,
        CancellationToken cancellationToken = default)
    {
        var resolvedRaw = await PayloadInputResolver.ResolveRawAsync(correctedRaw, correctedRawPath, cancellationToken);
        var resolvedOriginalId = await refResolver.ResolveMemoryIdAsync(originalId, cancellationToken);
        var result = await memoryStore.ReviseMemoryAsync(resolvedOriginalId, resolvedRaw, memoryFrom, note, copyEntityLinks, cancellationToken);
