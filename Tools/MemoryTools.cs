...
[McpServerToolType]
public class MemoryTools(MemoryStoreService memoryStore)
{
    [McpServerTool, Description("Store a raw memory observation. The raw text is never modified after creation.")]
    ...
    public async Task<string> GetMemory(
        [Description("Memory identifier.")] Guid id,
        CancellationToken cancellationToken = default)
    {
        var result = await memoryStore.GetMemoryAsync(id, cancellationToken);
    ...
    public async Task<string> UpdateMemoryFrom(
        ...
        CancellationToken cancellationToken = default)
    {
        var result = await memoryStore.UpdateMemoryFromAsync(id, memoryFrom, note, cancellationToken);
        return JsonResult.Ok(result);
    }
    ...
    public async Task<string> InvalidateMemory(
        ...
        CancellationToken cancellationToken = default)
    {
        var result = await memoryStore.InvalidateMemoryAsync(id, status, note, cancellationToken);
        return JsonResult.Ok(result);
    }
    ...
    public async Task<string> ReviseMemory(
        ...
        CancellationToken cancellationToken = default)
    {
        var result = await memoryStore.ReviseMemoryAsync(originalId, correctedRaw, memoryFrom, note, copyEntityLinks, cancellationToken);
        return JsonResult.Ok(result);
    }
    ...
    public async Task<string> GetMemoryHistory(
        ...
        CancellationToken cancellationToken = default)
    {
        var result = await memoryStore.GetMemoryHistoryAsync(id, cancellationToken);
        return result is null ? JsonResult.Ok(new { error = "Memory not found." }) : JsonResult.Ok(result);
    }
}
